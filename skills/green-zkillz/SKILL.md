---
name: green-zkillz
description: Run the Green-Zkillz adaptive skill pipeline through green-probe, green-bootstrap, green-ingest, green-format, and green-deploy.
metadata:
  type: workflow
  version: "1.2"
  package: green-zkillz
  project: green-agentz
---

# Green-Zkillz Pipeline

Execute the five-skill pipeline defined by
`../../docs/green-zkillz/REQUIREMENTS.md`. Do not invent extra ceremony.
...
Read `references/sys-requirements.md` and obey REQ-SYS-01 through REQ-SYS-06
on every step. Identity and URLs come from `HOST-BINDINGS.md`; empty site
fields mean ask.

Workspace root defaults to the current project directory. Set
`GREEN_WORKSPACE` before running scripts.
...
Do not compress human prose or comments. Intern STATUS enums, compiler
diagnostics, and path stems only.

- Static tables: `assets/gdict-1.0.0.txt`, `assets/gdict-errors-1.0.0.txt`.
- Session/user LRU + counting Bloom: `scripts/gdict-lru.sh`.
...
- Usage ledger: `scripts/gdict-lru.sh usage|record|usage-config`. See `references/REQUIREMENTS-CODEC.md`.
